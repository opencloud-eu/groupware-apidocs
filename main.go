...
func endpoints(base string, stmts []ast.Stmt, pkg string) ([]Endpoint, error) {
	result := []Endpoint{}
...
func scrapeGlobalParamDecls(decls []ast.Decl) (GlobalParamDecls, error) {
	...
	for _, decl := range decls {
		for _, c := range consts(decl) {
			if hasAnyPrefix(c.Name, PathParamPrefixes) {
				desc := describeParam(c.Comments)
				pathParams[c.Name] = Param{Name: c.Value, Description: desc, Required: true}
			} else if hasAnyPrefix(c.Name, QueryParamPrefixes) {
				desc := describeParam(c.Comments)
				queryParams[c.Name] = Param{Name: c.Value, Description: desc, Required: false}
			} else if hasAnyPrefix(c.Name, HeaderParamPrefixes) {
				desc := describeParam(c.Comments)
				headerParams[c.Name] = Param{Name: c.Value, Description: desc, Required: false}
			}
		}
	}
...
func (v returnVisitor) resolve(x *ast.Ident) (Type, error) {
	...
	//return v.resolveType(z.Type())
	return typeOf(z.Type(), v.typeMap), nil
}
...
func newParamsVisitor(fset *token.FileSet, pkg *packages.Package, fun string, typeMap map[string]Type, responseFuncs map[string]responseFunc) paramsVisitor {
	...
	if s, ok := isSelector(call.Fun); ok {
		if x, ok := isIdent(s.X); ok && len(call.Args) == 0 && x.Obj != nil {
			if m := need.FindAllStringSubmatch(s.Sel.Name, 2); m != nil {
				if f, ok := isField(x.Obj.Decl); ok {
					if t, ok := isIdent(f.Type); ok && t.Name == "Request" {
						return AccountIdUriParamName, "", true
					}
				}
			}
		}
	...
	case *ast.CallExpr:
		if t, ok, err := v.isBodyCall(d); err != nil {
			*v.errs = append(*v.errs, err)
			...
			v.pathParams[AccountIdUriParamName] = Param{Name: AccountIdUriParamName, Description: "", Required: true}
		} else if r, ok := v.isRespondCall(d); ok {
			maps.Copy(v.responseTypes, r)
		} else if z, desc, ok := v.isNeedAccountCall(d); ok {
			v.pathParams[z] = Param{Name: z, Description: desc, Required: true}
		} else if z, desc, ok := v.isPathParamCall(d); ok {
			v.pathParams[z] = Param{Name: z, Description: desc, Required: true}
		} else if z, desc, ok := v.isParseQueryParamCall(d); ok {
			v.queryParams[z] = Param{Name: z, Description: desc, Required: false}
		} else if z, desc, req, ok := v.isHeaderParamCall(d); ok {
			v.headerParams[z] = Param{Name: z, Description: desc, Required: req}
		}
	}
	return v
}
...
func isRouteFun(f *types.Func) bool {
	...
	if slices.Contains(MiddlewareFunctionNames, f.Name()) {
		return false
	}
	s := f.Signature()
...
func main() {
	...
	ims := []Impl{}
	{
		...
		pkgs, err := packages.Load(cfg, SourceDirectories...)
		if err != nil {
			log.Fatal(err)
		}
	...
	defaultResponseHeaders := map[string]ResponseHeaderDesc{} // TODO use a struct instead of string to indicate more information (required or not)
	{
		// defaultHeaders["Content-Language"]
		// defaultHeaders["ETag"]
		defaultResponseHeaders["Session-State"] = ResponseHeaderDesc{Summary: "The opaque state identifier for the JMAP Session", Required: true}
		defaultResponseHeaders["State"] = ResponseHeaderDesc{Summary: "The opaque state identifier for the type of objects in the response"}
		defaultResponseHeaders["Object-Type"] = ResponseHeaderDesc{Summary: "The type of JMAP objects returned in the response"}
		defaultResponseHeaders["Account-Id"] = ResponseHeaderDesc{Summary: "The identifier of the account the operation was performed against, when against a single account"}
		defaultResponseHeaders["Account-Ids"] = ResponseHeaderDesc{Summary: "The identifier of the accounts the operation was performed against, when against multiple accounts", Explode: true}
		defaultResponseHeaders["Trace-Id"] = ResponseHeaderDesc{Summary: "The value of the Trace-Id header that was specified in the request or, if not, a unique randomly generated identifier that is included in logging output", Required: true}
	}

	commonRequestHeaders := []RequestHeaderDesc{}
	{
		commonRequestHeaders = append(commonRequestHeaders, RequestHeaderDesc{
			Name:        "X-Request-ID",
